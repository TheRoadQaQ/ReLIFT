...
from verl.utils.torch_functional import logprobs_from_logits, all_logprobs_from_logits, logprobs_from_logits_v2
...
class ReLIFTDataParallelPPOActor(BasePPOActor):
    ...
    def _forward_micro_batch_v2(self, micro_batch, temperature, calculate_entropy=False) -> Tuple[torch.Tensor, torch.Tensor]:
        """
        Returns:
            entropy: # (bs, response_len)
            log_probs: # (bs, response_len, vocab_size)
        """
        response_length = micro_batch["responses"].size(-1)
        multi_modal_inputs = {}
        if "multi_modal_inputs" in micro_batch.keys():
        ...
        with torch.autocast(device_type=self.device_name, dtype=torch.bfloat16):
            ...
            if self.use_remove_padding:
                ...
                if self.use_fused_kernels:
                    ...
                    log_probs = all_logprobs_from_logits(
                        logits=logits_rmpad
                    )
                ...
                if self.use_fused_kernels:
                    ...
                    logits.div_(temperature)
                    logits = logits[:, -response_length - 1 : -1, :]  # (bsz, response_length, vocab_size)
                    log_probs = logprobs_from_logits_v2(logits, micro_batch["responses"])
                    if calculate_entropy:
                        entropy = verl_F.entropy_from_logits(logits)  # (bsz, response_length)
