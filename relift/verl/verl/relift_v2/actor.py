...
class ReLIFTDataParallelPPOActor(BasePPOActor):
    ...
    @GPUMemoryLogger(role="dp actor", logger=logger)
    def relift_update_policy_v2(self, data: DataProto):
        # make sure we are in training mode
        self.actor_module.train()
        ...
        batch_sft_num = data.batch['sft_mask'].any(dim=1).sum().item()
        batch_rl_num = self.config.ppo_mini_batch_size - batch_sft_num
        sft_weight = batch_sft_num / self.config.ppo_mini_batch_size
        rl_weight = batch_rl_num / self.config.ppo_mini_batch_size

        print(sft_weight, rl_weight)
        ...
        for epoch in range(self.config.ppo_epochs):
            for batch_idx, data in enumerate(dataloader):
                ...
                for data in micro_batches:
                    ...
                    policy_loss = rl_weight * pg_loss + sft_weight * sft_loss
