...
class ReLIFTActorRolloutRefWorker(Worker):
    ...
    def init_model(self):
        ...
        if self._is_actor:
            ...
            self.sft_checkpoint_manager = None
    ...
    @register(dispatch_mode=Dispatch.DP_COMPUTE_PROTO)
    def update_actor_relift(self, data: DataProto, sft_data: DataProto):
        # Support all hardwares
        data = data.to(get_torch_device().current_device())
        sft_data = sft_data.to(get_torch_device().current_device())
        ...
        with self.ulysses_sharding_manager:
            ...
            # perform training
            with Timer(name="update_policy", logger=None) as timer:
                metrics = self.actor.update_policy_relift(data=data, sft_data=sft_data)
            delta_time = timer.last
            global_num_tokens = data.meta_info["global_token_num"]
            estimated_flops, promised_flops = self.flops_counter.estimate_flops(global_num_tokens, delta_time)
            metrics["perf/mfu/actor"] = estimated_flops * self.config.actor.ppo_epochs / promised_flops / self.world_size
